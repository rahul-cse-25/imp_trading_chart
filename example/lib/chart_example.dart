...
import 'package:imp_trading_chart/imp_trading_chart.dart' show Candle, dummy, parseCandlesFromPayload, ImpChart;
...
class _ChartExampleScreenState extends State<ChartExampleScreen> with SingleTickerProviderStateMixin {
    ...
    void _generateCandlesForMode(SimulationMode mode) {
        ...
        for (int i = 0; i < 500; i++) {
            ...
            final changePercent = (_random.nextDouble() - 0.5) * 2.0 * volatilityPercent;
            ...
            final candleRange = math.max((close - open).abs(), price * volatilityPercent * 0.5);
            final high = math.max(open, close) + _random.nextDouble() * candleRange * 0.5;
            final low = math.min(open, close) - _random.nextDouble() * candleRange * 0.5;
    ...
    void _loadDummyData() {
        try {
            ...
            if (dummyMap['data'] == null || (dummyMap['data'] as Map<String, dynamic>)['chart_data'] == null) {
                debugPrint('Error: Dummy data structure is invalid - missing data.chart_data');
                return;
            }

            final chartDataList = (dummyMap['data'] as Map<String, dynamic>)['chart_data'] as List;
            ...
            debugPrint('✓ Successfully loaded ${sortedCandles.length} candles from dummy data');
            debugPrint('  First candle: time=${sortedCandles.first.time}, close=${sortedCandles.first.close}');
            debugPrint('  Last candle: time=${sortedCandles.last.time}, close=${sortedCandles.last.close}');

            final minPrice = sortedCandles.map((c) => c.low).reduce((a, b) => a < b ? a : b);
            final maxPrice = sortedCandles.map((c) => c.high).reduce((a, b) => a > b ? a : b);
    ...
    void _startLiveUpdates(SimulationMode mode) {
        Future.delayed(const Duration(milliseconds: 500), () {
                ...
                setState(() {
                        ...
                        double minPriceValue = mode == SimulationMode.low || mode == SimulationMode.mixed 
                            ? 0.00000000000000000000001 
                            : 0.01;
                        final newPrice = (currentPrice + priceChange).clamp(minPriceValue, maxPrice * 1.1);

                        // For mixed mode, occasionally jump to a completely different range
                        if (mode == SimulationMode.mixed && _random.nextDouble() < 0.02) { // 2% chance
                            final segment = _random.nextInt(3);
                            double jumpPrice;
                            switch (segment) {
                        ...
                        if (lastCandle.time == newTime) {
                            ...
                            final candleRange = math.max((newPrice - lastCandle.close).abs(), currentPrice * volatilityPercent * 0.5);
                            final high = math.max(lastCandle.close, newPrice) + _random.nextDouble() * candleRange * 0.3;
                            final low = math.min(lastCandle.close, newPrice) - _random.nextDouble() * candleRange * 0.3;
    ...
    Widget _buildSimulationButton(String label, SimulationMode mode, Color color, bool isActive) {
        final isLive = (mode == SimulationMode.low && _isLiveLow) ||
            (mode == SimulationMode.medium && _isLiveMedium) ||
            (mode == SimulationMode.high && _isLiveHigh);
