...
class ChartEngine {
  ...
  static ChartViewport _defaultViewport(
    ...
    if (totalCount == 0) {
      return ChartViewport(
          startIndex: 0, visibleCount: visibleCount, totalCount: 0);
    }
  ...
  PriceScale getPriceScale({double paddingPercent = 0.05}) {
    ...
    if (_useCloseOnlyPriceScale) {
      ...
      _cachedScale = PriceScale.fromCandlesCloseOnly(visibleCandles,
          paddingPercent: paddingPercent);
    } else {
      // Candlestick chart: use high/low prices
      _cachedScale = PriceScale.fromCandles(visibleCandles,
          paddingPercent: paddingPercent);
    }
  ...
  ChartEngine zoom(int delta, {int minVisible = 5, int maxVisible = 1000}) {
    return withViewport(
        _viewport.zoom(delta, minVisible: minVisible, maxVisible: maxVisible));
  }

  /// Zoom around a specific index
  ChartEngine zoomAround(int anchorIndex, int delta,
      {int minVisible = 5, int maxVisible = 1000}) {
    return withViewport(_viewport.zoomAround(anchorIndex, delta,
        minVisible: minVisible, maxVisible: maxVisible));
  }
  ...
  (ChartEngine, bool) handleTick(int time, double price) {
    ...
    if (lastCandle.time == time) {
      ...
      if (shouldScroll) {
        return (
          newEngine.resetViewport(visibleCount: _viewport.visibleCount),
          true
        );
      }
