...
import 'package:imp_trading_chart/imp_trading_chart.dart'
    show Candle, ChartStyle, PriceLineStyle;
...
class ChartPainter extends CustomPainter {
    ...
    if (horizontalLines > 0) {
      for (int i = 0; i <= horizontalLines; i++) {
        ...
        _drawStyledLine(
          ...
          Offset(
              clampedHorizontalEndX, y), // End: same Y, extends toward labels
          paint,
          gridLineStyle,
        );
      }
      ...
      for (int i = 0; i <= verticalLines; i++) {
        ...
        if (x.isFinite && x >= chartLeft && x <= chartRight) {
          ...
          _drawStyledLine(
            ...
            Offset(
                x, clampedVerticalEndY), // End: same X, extends toward labels
            paint,
            gridLineStyle,
          );
        }
  ...
  void _drawSingleDataPoint(Canvas canvas) {
    ...
    final y =
        mapper.priceToY(candle.close); // Use price scale for Y positioning
  ...
  Path _generateCardinalSplinePath(List<Offset> points) {
    ...
    final tension =
        1.0 - userTension; // Invert: high curveTension = low cardinal tension
  ...
  void _drawTimeLabels(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < labelInfos.length; i++) {
      ...
      if (candles.length == 1 && labelInfos.length == 1) {
        ...
        final centeredLabelX = (centerX - (label.width / 2))
            .clamp(chartLeft, chartRight - label.width);
      ...
      for (final visible in visibleLabels) {
        ...
        if ((labelLeft < visibleRight + minSpacing) &&
            (labelRight + minSpacing > visibleLeft)) {
          overlaps = true;
          break;
        }
      }
  ...
  void _drawCrosshair(Canvas canvas, Size size) {
    ...
    if (cs.showPriceLabel) {
      ...
      final priceRRect = RRect.fromRectAndRadius(
          priceBgRect, Radius.circular(cs.labelBorderRadius));
    ...
    if (cs.showTimeLabel) {
      ...
      final timeRRect = RRect.fromRectAndRadius(
          timeBgRect, Radius.circular(cs.labelBorderRadius));
