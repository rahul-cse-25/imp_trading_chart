...
import 'package:imp_trading_chart/imp_trading_chart.dart' show Candle, ChartStyle;
...
import 'package:imp_trading_chart/src/rendering/chart_painter.dart' show ChartPainter;
...
class _ImpChartState extends State<ImpChart> with SingleTickerProviderStateMixin {
    ...
    void didUpdateWidget(ImpChart oldWidget) {
        ...
        final candleCountChanged = widget.candles.length != oldWidget.candles.length;
        ...
        if (candlesChanged || lastCandleChanged) {
            ...
            if (oldCandleCount > 0 && newCandleCount > 0) {
                isCompletelyNewData = widget.candles.first.time != oldWidget.candles.first.time;
            }

            // Update engine with new candles
            var newEngine = _engine.withCandles(widget.candles, defaultVisibleCount: widget.defaultVisibleCount);
            ...
            if (isDataReplacement || isCompletelyNewData) {
                ...
                newEngine = newEngine.withViewport(
                    ChartViewport.last(targetVisibleCount.clamp(1, newCandleCount), newCandleCount),
                );
            ...
            // Check if new candle was added (not just updated)
                ...
                if (wasNearEnd) {
                    ...
                    final visibleCount = math.max(currentVisibleCount, math.min(targetVisibleCount, newCandleCount));
                    newEngine = newEngine.withViewport(
                        ChartViewport.last(visibleCount, newCandleCount),
                    );
                }
                _updateEngine(newEngine);
            } else if (newCandleCount == oldCandleCount && (candlesChanged || lastCandleChanged)) {
        ...
        final rippleEnabled = widget.style.rippleStyle.show && widget.candles.isNotEmpty;
        final wasRippleEnabled = oldWidget.style.rippleStyle.show && oldWidget.candles.isNotEmpty;
    ...
    void _startContinuousRipple() {
        ...
        _pulseController.duration = Duration(milliseconds: rippleStyle.animationDurationMs);
        ...
        final totalCycleMs = rippleStyle.animationDurationMs + rippleStyle.intervalMs;
        _rippleTimer = Timer.periodic(Duration(milliseconds: totalCycleMs), (_) {
                if (mounted && widget.candles.isNotEmpty && widget.style.rippleStyle.show) {
                    _playRippleAnimation();
                }
            });
    }
    ...
    void _playRippleAnimation() {
        if (!mounted || !widget.style.rippleStyle.show || widget.candles.isEmpty) return;
    ...
    void _handleScaleUpdate(ScaleUpdateDetails details, Size size) {
        ...
        final isZoom = scaleChange > 0.05; // Higher threshold to avoid false zoom detection
        ...
        if (isZoom) {
            ...
            if (candleWidth > 0) {
                ...
                if (candleDelta.abs() >= 1) {
                    ...
                    _accumulatedPanDelta = _accumulatedPanDelta - (candleDelta * candleWidth);
                }
            }
    ...
    void _handleDoubleTap() {
        ...
        _updateEngine(_engine.resetViewport(visibleCount: widget.defaultVisibleCount ?? 100));
    }
    ...
    void _handleLongPressMoveUpdate(LongPressMoveUpdateDetails details, Size size) {
        if (!widget.enableGestures || !widget.style.crosshairStyle.show) return;
        _updateCrosshair(details.localPosition, size);
    }
    ...
    _PaddingInfo _calculatePadding(Size size) {
        ...
        if (priceLabelStyle.show && _engine.candles.isNotEmpty) {
            ...
            if (currentPriceStyle.showLabel && _engine.candles.isNotEmpty) {
                ...
                if (currentPrice != null) {
                    ...
                    textPainter.layout();
                    currentPriceLabelWidth = textPainter.width + (currentPriceStyle.labelPaddingH * 2);
                }
            }

            // Y-axis area = gap + max(label width, current price label width) + padding
            final yAxisLabelPadding = layout.yAxisLabelPadding;
            final effectiveLabelWidth = math.max(maxLabelWidth, currentPriceLabelWidth);
            yAxisAreaWidth = layout.yAxisGap + effectiveLabelWidth + yAxisLabelPadding.horizontal;
        }
        ...
        if (timeLabelStyle.show && _engine.candles.isNotEmpty) {
            ...
            xAxisAreaHeight = layout.xAxisGap +
                textPainter.height +
                xAxisLabelPadding.vertical;
        }

        return _PaddingInfo(
            left: chartDataLeft,
            right: yAxisAreaWidth > 0 ? yAxisAreaWidth : layout.chartDataPadding.right,
            top: chartDataTop,
            bottom: xAxisAreaHeight > 0 ? xAxisAreaHeight : layout.chartDataPadding.bottom,
        );
    }
            ...
            child: LayoutBuilder(
                builder: (context, constraints) {
                    ...
                    return GestureDetector(
                        onScaleStart: widget.style.crosshairStyle.show ? null : (details) => _handleScaleStart(details, size),
                        onScaleUpdate: widget.style.crosshairStyle.show ? null : (details) => _handleScaleUpdate(details, size),
                        onDoubleTap: widget.style.crosshairStyle.show ? null : _handleDoubleTap,
                        onLongPressStart: widget.style.crosshairStyle.show ? (details) => _handleLongPressStart(details, size) : null,
                        onLongPressMoveUpdate: widget.style.crosshairStyle.show ? (details) => _handleLongPressMoveUpdate(details, size) : null,
                        onLongPressEnd: widget.style.crosshairStyle.show ? (_) => _handleLongPressEnd() : null,
                        child: CustomPaint(
